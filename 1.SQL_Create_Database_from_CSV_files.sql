...
--We are going to:
    ...
    --STEP 3: Create a new table bike_pre, copy of bike_raw, that is going to be use to preprocess and clean the data.

--STEP 1: Creating bike_raw table. Data types are being set accordingly, some may be changed later on after checking the data:

    CREATE TABLE bike_raw (
        ride_id VARCHAR(50),
        rideable_type VARCHAR(20), 
        started_at TIMESTAMP, -- datatime value
        ended_at TIMESTAMP, -- datatime value
...
--STEP 2: Importing Data from CSV files. 
    ...
    --We succesfully created a bike_raw table, that has all the raw data from the csv files

--STEP 3: Creating bike_pre table, a copy of bike_raw table. We want to keep the raw data intact in bike_raw.

    --Creating bike_pre table from bike_raw
    CREATE TABLE bike_pre AS
    SELECT *
    FROM bike_raw;
    --DONE. We created bike_pre table. Bike pre is going to be used to preprocess and clean the data.

--The process of importing the data, and creating a new table with all raw values is complete. Next, go to 2.SQL_Cleaning_and_processing_data.csv 